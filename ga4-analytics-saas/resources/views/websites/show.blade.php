<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div>
                <a href="{{ route('websites.edit', $website->id) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded mr-2">
                    編集
                </a>
                <a href="{{ route('websites.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                    一覧に戻る
                </a>
            </div>
        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            @if ($website->analyticsAccount)
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('google.analytics.redirect', $website->id) }}" class="text-sm text-blue-600 hover:underline">再接続</a>
                                    <form method="POST" action="#" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-sm text-red-600 hover:underline" onclick="return confirm('本当に接続を解除しますか？')">
                                            接続解除
                                        </button>
                                    </form>
                                </div>
                            @else
                                ...
                                <a href="{{ route('google.analytics.redirect', $website->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-bold py-2 px-4 rounded">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            @if ($website->searchConsoleAccount)
                                ...
                                <div class="flex space-x-2">
                                    <a href="{{ route('google.searchconsole.redirect', $website->id) }}" class="text-sm text-blue-600 hover:underline">再接続</a>
                                    <form method="POST" action="#" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-sm text-red-600 hover:underline" onclick="return confirm('本当に接続を解除しますか？')">
                            ...
                            @else
                                ...
                                <a href="{{ route('google.searchconsole.redirect', $website->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-bold py-2 px-4 rounded">
                                    Search Consoleと接続する
                                </a>
                            @endif
                        </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <a href="{{ route('reports.create', $website->id) }}?type=executive" class="block p-4 bg-blue-50 hover:bg-blue-100 rounded-lg transition duration-150">
                                <h4 class="font-medium text-blue-800 mb-2">経営者向けレポート</h4>
                                <p class="text-sm text-gray-600">ビジネス指標に焦点を当てた包括的な分析レポート</p>
                            </a>

                            <a href="{{ route('reports.create', $website->id) }}?type=technical" class="block p-4 bg-green-50 hover:bg-green-100 rounded-lg transition duration-150">
                                <h4 class="font-medium text-green-800 mb-2">技術者向けレポート</h4>
                                <p class="text-sm text-gray-600">サイトパフォーマンスや技術的な問題に焦点を当てた分析</p>
                            </a>

                            <a href="{{ route('reports.create', $website->id) }}?type=content" class="block p-4 bg-purple-50 hover:bg-purple-100 rounded-lg transition duration-150">
            ...
            @else
                <div class="bg-yellow-50 overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="flex items-center">
                        ...
                        <p class="text-yellow-800 font-medium">データ分析を利用するには、まずGoogle AnalyticsとSearch Consoleを接続してください。</p>
                    </div>
                </div>
            @endif
        </div>
